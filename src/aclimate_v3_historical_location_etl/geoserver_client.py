...
class GeoServerClient:
    ...
    def __init__(self, geoserver_config: Dict[str, Any]):
        ...
        info("GeoServer client initialized", 
             component="geoserver_client")
    
    def extract_location_data(self, location_ids: str, country: str, 
        ...
        """
        try:
            info(f"Starting location data extraction from {start_date} to {end_date}",
                 component="geoserver_client",
            ...
            if not locations:
                error(f"No locations found for criteria {location_ids} in {country}", 
                      component="geoserver_client",
                      location_ids=location_ids,
                      country=country)
                sys.exit(1)
            ...
            with DownloadProgressBar(
                ...
                for date in date_list:
                    for location in locations:
                        location_data, extraction_stats = self._extract_location_point_data(
            ...
            info(f"Location data extraction completed from {start_date} to {end_date} - {successful_extractions} successful, {failed_extractions} failed",
                 component="geoserver_client",
            ...
            cleaned_df, validation_results = self.data_validator.validate_extracted_data(
                df=df,
            ...
            # Log validation completion with report in the main message for console visibility
            info(f"Data validation completed\n{validation_report}",
                 component="geoserver_client",
            ...
            if not validation_results['is_valid']:
                ...
                print(f"\n❌ ERROR: Data validation failed with {len(validation_results['errors'])} critical errors:")
                for i, error_msg in enumerate(validation_results['errors'], 1):
                    print(f"   {i}. {error_msg}")
                print("\nProcessing cannot continue. Please review the validation report above.\n")
                
                # Also log critical validation failure with structured data
                error("Data validation failed - cannot continue with processing",
                      component="geoserver_client",
                      errors_count=len(validation_results['errors']),
                      warnings_count=len(validation_results['warnings']),
            ...
            info("Data validation and cleaning passed successfully",
                 component="geoserver_client",
                 total_records=validation_results['statistics'].get('total_records', len(cleaned_df)),
                 warnings_count=len(validation_results['warnings']),
                 validation_warnings=validation_results['warnings'] if validation_results['warnings'] else None,
                 validation_statistics=validation_results['statistics'],
                 cleaning_actions_count=len(validation_results.get('cleaning_actions', [])))
            
            return cleaned_df
            
        except Exception as e:
            error("Failed to extract location data",
                  component="geoserver_client",
                  error=str(e))
            sys.exit(1)
    
    def _extract_location_point_data(self, location: LocationRead, date: datetime, country: str, pbar: DownloadProgressBar = None) -> tuple[Dict[str, Any], Dict[str, Any]]:
        """
        ...
        try:
            ...
            if pbar:
                pbar.set_description(f"Extracting {location.name} - {date.strftime('%Y-%m-%d')}")
                pbar.update(1)
            ...
            daily_data, download_count = self._extract_daily_data(location, date, country)
            ...
            error("Failed to extract point data for location",
                  component="geoserver_client",
                  location_id=location.id,
                  date=date.strftime("%Y-%m-%d"),
                  error=str(e))
    ...
    def _extract_daily_data(self, location, date: datetime, country: str) -> tuple[Dict[str, Any], int]:
        """
        ...
        try:
            ...
            error(f"Failed to extract daily data for location {location.name} {str(e)}",
                  component="geoserver_client",
                  location_id=location.id,
                  date=date.strftime("%Y-%m-%d"),
                  error=str(e))
    ...
    def _extract_point_value(self, location, date: datetime, variable_name: str, 
                           layer_config: Dict[str, Any], country_config: Dict[str, Any]) -> tuple[Optional[float], int]:
        """
        ...
        try:
            ...
            tiff_data = self._download_from_geoserver(date, variable_name, layer_config, country_config)
            
            download_count = 1 if tiff_data is not None else 0
            
            if tiff_data is None:
                warning("Failed to download GeoTIFF",
                       component="geoserver_client",
            ...
            value = self._extract_point_from_tiff(tiff_data, location.longitude, location.latitude)
            ...
            error("Failed to extract point value",
                  component="geoserver_client",
                  location_id=location.id,
                  variable=variable_name,
                  date=date.strftime("%Y-%m-%d"),
    ...
    def _download_from_geoserver(self, date: datetime, variable_name: str, 
                               layer_config: Dict[str, Any], country_config: Dict[str, Any]) -> Optional[bytes]:
        """
        ...
        try:
            ...
            if not content.startswith(b'\x49\x49\x2A\x00') and not content.startswith(b'\x4D\x4D\x00\x2A'):
    ...
    def _extract_point_from_tiff(self, tiff_data: bytes, longitude: float, 
                               latitude: float) -> Optional[float]:
        """
        Extract point value from a GeoTIFF data using xarray with spatial interpolation fallback.
        ...
        try:
            ...
            try:
                ...
                # Check if the value is actually valid (not NaN and not a special nodata value)
                if not pd.isna(extracted_value) and extracted_value != -9999 and extracted_value != -999:
                    is_valid = True
            ...
            interpolated_value = self._interpolate_spatial_value(data_xarray, longitude, latitude)
            ...
            error(f"Failed to extract point from GeoTIFF {str(e)}",
                  component="geoserver_client",
                  longitude=longitude,
                  latitude=latitude,
                  error=str(e))
    ...
    def _interpolate_spatial_value(self, data_array: xr.DataArray, longitude: float, 
                                 latitude: float, search_radius: float = 0.1) -> Optional[float]:
        """
        ...
        try:
            ...
            for radius in search_radii:
                ...
                try:
                        ...
                        for x_idx in range(subset.sizes.get('x', 0)):
                            try:
                                ...
                                if not pixel_value.isnull():
                                    ...
                                    # Skip nodata values
                                    if not pd.isna(pixel_val) and pixel_val != -9999 and pixel_val != -999:
                    ...
                    if len(valid_values) >= 1:  # Need at least 1 valid pixel
                        ...
                        info(f"Spatial interpolation successful at radius {radius} km longitude={longitude}, latitude={latitude} interpolated_value={interpolated_value}",
                             component="geoserver_client",
                             longitude=longitude,
                             latitude=latitude,
            ...
            warning("No valid pixels found for interpolation",
                   component="geoserver_client",
                   longitude=longitude,
                   latitude=latitude,
            ...
            warning("Spatial interpolation failed",
                   component="geoserver_client",
                   longitude=longitude,
                   latitude=latitude,
                   error=str(e))
    ...
    def _inverse_distance_weighting(self, target_lon: float, target_lat: float,
                                  coords: List[tuple], values: List[float],
        ...
        try:
            ...
            for coord in coords:
                ...
                distance = ((target_lon - coord[0])**2 + (target_lat - coord[1])**2)**0.5
            ...
            error("Inverse distance weighting failed",
                  component="geoserver_client",
                  error=str(e))
