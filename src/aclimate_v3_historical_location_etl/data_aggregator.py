...
class DataAggregator:
    ...
    def calculate_monthly_aggregations(self, daily_data: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        try:
            info("Calculating monthly aggregations from daily data", 
                 component="data_aggregator",
                 total_records=len(daily_data))
            
            if daily_data.empty:
                warning("No daily data provided for aggregation", 
                       component="data_aggregator")
                return pd.DataFrame()
            ...
            for col in daily_data.columns:
                if col not in metadata_columns and pd.api.types.is_numeric_dtype(daily_data[col]):
                    climate_columns.append(col)
            
            info(f"Found climate variables for aggregation: {climate_columns}", 
                 component="data_aggregator",
                 climate_variables=climate_columns)
            
            # Group by location, year, and month, then calculate appropriate aggregation
            grouping_columns = ['location_id', 'location_name', 'latitude', 'longitude', 'year', 'month']
            ...
            for col in climate_columns:
                ...
                if 'prec' in col.lower() or 'precipitation' in col.lower():
                    agg_dict[col] = 'sum'
                    info(f"Using SUM aggregation for precipitation variable: {col}", 
                         component="data_aggregator", 
                         variable=col,
                    ...
                    agg_dict[col] = 'mean'
                    info(f"Using MEAN aggregation for variable: {col}", 
                         component="data_aggregator", 
                         variable=col,
                         aggregation_type='mean')
            
            # Calculate monthly aggregations
            monthly_data = daily_data.groupby(grouping_columns).agg(agg_dict).reset_index()
            ...
            info("Monthly aggregations calculated successfully", 
                 component="data_aggregator",
            ...
            error("Failed to calculate monthly aggregations", 
                  component="data_aggregator",
                  error=str(e))
            raise DataAggregatorError(f"Failed to calculate monthly aggregations: {str(e)}")