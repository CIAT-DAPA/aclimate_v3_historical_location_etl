...
class DataValidator:
    ...
    def _validate_completeness(
        ...
        if "location_id" in df.columns:
            ...
            location_coverage = (
                len(actual_locations & expected_locations_set)
                / len(expected_locations_set)
                * 100
            )

            if location_coverage < 90:
                errors.append(
                    f"Insufficient location coverage: {location_coverage:.1f}%"
                )
            elif location_coverage < 100:
                warnings.append(
                    f"Incomplete location coverage: {location_coverage:.1f}%"
                )
    ...
    def _validate_quality(self, df: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        for col in data_columns:
            ...
            if completeness == 0:
                errors.append(
                    f"CRITICAL: Variable '{col}' is completely empty across "
                    f"all locations and dates"
                )
            elif completeness < 30:
                errors.append(
                    f"CRITICAL: Variable '{col}' has very low completeness: "
                    f"{completeness:.1f}% (interpolation largely failed)"
                )
            elif completeness < 70:
                warnings.append(
                    f"Variable '{col}' has low completeness: {completeness:.1f}%"
                )
            elif completeness < 95:
                warnings.append(
                    f"Variable '{col}' has minor missing data: {completeness:.1f}%"
                )

        # Location-specific validation with flexible approach
        location_errors, location_warnings = self._validate_locations_quality_flexible(
            df, data_columns
        )
    ...
    def _validate_locations_quality_flexible(
        self, df: pd.DataFrame, data_columns: List[str]
    ) -> Tuple[List[str], List[str]]:
        """
        Validate quality per location with flexible criteria for mixed
        ...
        for location_id in df["location_id"].unique():
            ...
            location_name = (
                location_data["location_name"].iloc[0]
                if not location_data.empty
                else f"Location {location_id}"
            )
                ...
                if valid_count > 0:
                    ...
                    if completeness < 95:
                        warnings.append(
                            ...
                            f"variable '{col}' has {completeness:.1f}% completeness "
                            f"({valid_count}/{total_count} records)"
                        )
                else:
            ...
            if not critical_vars_available:
                errors.append(
                    f"Location '{location_name}' (ID: {location_id}) has NO "
                    f"critical climate variables available"
                )

            # Error for completely missing variables (interpolation failed)
            if missing_vars:
                errors.append(
                    f"Location '{location_name}' (ID: {location_id}) has "
                    f"completely missing variables (interpolation failed): "
                    f"{', '.join(missing_vars)}"
                )
            ...
            # Warning for limited variable availability but at least some data exists
                ...
                warnings.append(
                    f"Location '{location_name}' (ID: {location_id}) has only "
                    f"1 variable available: {', '.join(available_vars)}"
                )
            elif (
                len(available_vars) < len(data_columns) * 0.5
            ):  # Less than 50% of expected variables
                warnings.append(
                    f"Location '{location_name}' (ID: {location_id}) has "
                    f"limited variables "
                    f"({len(available_vars)}/{len(data_columns)}): "
        ...
        for col in data_columns:
            if pd.api.types.is_numeric_dtype(df[col]):
                ...
                if len(valid_data) > 0:
                    ...
                    IQR = Q3 - Q1
                    outliers = valid_data[
                        (valid_data < Q1 - 1.5 * IQR) | (valid_data > Q3 + 1.5 * IQR)
                    ]
                    outlier_pct = (len(outliers) / len(valid_data)) * 100

                    if outlier_pct > 5:
                        warnings.append(
                            f"High outlier percentage for {col}: {outlier_pct:.1f}%"
                        )
