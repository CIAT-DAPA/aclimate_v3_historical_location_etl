...
class LoggingManager:
    ...
    def __init__(
        ...
        self.log_file: str = log_file or os.getenv("LOG_FILE_PATH") or "application.log"
    ...
    def _try_initialize_signoz(self, formatter: logging.Formatter) -> None:
        ...
        try:
            ...
            exporter = OTLPLogExporter(endpoint=self.endpoint, insecure=True, timeout=5)

            logger_provider.add_log_record_processor(
                BatchLogRecordProcessor(
                    exporter, schedule_delay_millis=5000, max_export_batch_size=50
                )
            )
            ...
            signoz_handler = SigNozLogHandler(
                logger_provider=logger_provider, level=logging.INFO
            )
            ...
            self.logger.info(
                "SigNoz configured successfully", extra={"component": "logging_setup"}
            )
    ...
    # Convenience methods
    def info(
        self, message: str, component: Optional[str] = None, **kwargs: Any
    ) -> None:
